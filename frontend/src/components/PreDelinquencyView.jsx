      ...
      {metrics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Total Kasus Terdeteksi</p>
              <p className="text-2xl font-bold text-slate-900 mt-1">{metrics.total_cases} <span className="text-xs font-normal text-slate-500">Akun DPD 0</span></p>
              <p className="text-xs text-blue-600 mt-0.5">H-0 s.d H-3 Jatuh Tempo</p>
            </div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Terselesaikan (Cured / Sent)</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{metrics.cured_cases} <span className="text-xs font-normal text-slate-500">({metrics.resolution_rate.toFixed(1)}%)</span></p>
              <p className="text-xs text-slate-500 mt-0.5">Top-up Mandiri & WA Respon</p>
            </div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Potensi Tunggakan Dicegah</p>
              <p className="text-2xl font-bold text-indigo-700 mt-1">{formatIDR(metrics.total_potential_overdue)}</p>
              <p className="text-xs text-indigo-500 mt-0.5">Pencegahan Roll-Rate NPL</p>
            </div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Estimasi Efisiensi Biaya</p>
              <p className="text-2xl font-bold text-amber-600 mt-1">{formatIDR(metrics.estimated_cost_saving)}</p>
              <p className="text-xs text-slate-500 mt-0.5">Hemat Biaya Field & Telepon</p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-800">Daftar Akun Pengawasan Pre-Delinquency (DPD 0)</h3>
            <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium">
              {filteredAccounts.length} Akun
            </span>
          </div>
          <span className="text-xs text-slate-400">Data terhubung live dengan Core Banking CASA Engine</span>
        </div>
